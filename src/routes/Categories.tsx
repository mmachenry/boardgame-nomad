...
const Categories = () => {
  ...
  const updateListIndex = (event: SelectChangeEvent<number>) => {
  ...
  return (
    <>
      ...
      <Card>
        <CardContent>
          <List>
            {scattegoriesLists[listIndex].map((c,i) => (
              <ListItem key={i}>{i+1}.{c}</ListItem>
            ))}
          </List>
        </CardContent>
        <CardActions>
          <Select value={listIndex} onChange={updateListIndex}>
            {scattegoriesLists.map((l,i) => ( 
              <MenuItem key={i} value={i}>List {i+1}</MenuItem>
            ))}
          </Select>
        </CardActions>
      </Card>
